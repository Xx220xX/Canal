...
class QR{
    constructor(msg){
        this.msg = msg;
        this.modo_correcao_forcado = false;
        ...
        try{
          ...
          /*  this.dividir_em_blocos()*/
            ...
            if (!this.modo_correcao_forcado && this.modo_correcao > L){
                ...
                this.codificar('repeat');
                return;
               }
            print(e)
               throw  Error("mensagem nao suportada"); 
    	}
    }
    determinarMenorVersao(){
    	for(let i=0;i<40;i++){
   	    	if(this.msg.length <=tabela.vesoes[this.modo_correcao-1][i][this.modo_codificacao]){
	    		this.capacidade = tabela.vesoes[this.modo_correcao-1][i][this.modo_codificacao];
	    		this.versao = i+1;
	    		return;
	    	}
    	}
